...
# begin
# 	data = let
# 		datafile = Downloads.download("https://github.com/Antomek/IandH_data/raw/main/3Class_DendriteLayer_data.bson")
# 		BSON.load(datafile, @__MODULE__)[:data]
# 	end
# 	params = let
# 		modelfile = Downloads.download("https://github.com/Antomek/IandH_data/raw/main/3Class_DendriteLayer_parameters.bson")
# 		BSON.load(modelfile, @__MODULE__)[:params]
# 	end
# end

# ╔═╡ 74920fc8-c999-4e66-a82d-4a7c53501492
begin
	data = let
		datafile = Downloads.download("https://github.com/Antomek/IandH_data/raw/main/3Class_DendriteLayer_array_data.bson")
		BSON.load(datafile, @__MODULE__)[:data]
	end
	# params = let
	# 	modelfile = Downloads.download("https://github.com/Antomek/IandH_data/raw/main/3Class_DendriteLayer_array_parameters.bson")
	# 	BSON.load(modelfile, @__MODULE__)[:params]
	# end
	params = BSON.load("3Class_DendriteLayer_array_parameters.bson", @__MODULE__)[:params]
end
...
# ╔═╡ c15147c6-7347-4aff-afd0-214c48c2c50b
f(x) = x < 1 ? 0f0 : 5f0

# ╔═╡ 17d651e3-5f34-4e28-b661-02e3cb31331a
params3 = let
	ps = []
	for (i, (W, b)) in enumerate(zip(params[1:2:length(params)], params[2:2:length(params)]))
		i == 1 ? push!(ps, f.(W)) : push!(ps, clamp.(W, 0f0, Inf32))
		push!(ps, b)
	end
	ps
end

# ╔═╡ 44682525-d5e1-4536-a604-fe732d31ec15
testBNN = build_binary_network2(params2)

# ╔═╡ 1533d87b-328b-4177-a905-2c71d7a93e69
[l.weight for l in testBNN]

# ╔═╡ b3a85705-13cf-434c-a905-cff1a6c3f5b1
Array(params[1])

# ╔═╡ 2c3f6f89-3ffa-4b9c-98e7-23d8b3db2d5f
begin
	accuracy(m, x, y) = mean(onecold(m(x)) .== onecold(y));
	aux_BNN = build_binary_network2(params)
...
# ╔═╡ a9a94a53-7fcb-4fd4-ae2a-708e77bb633c
accuracy(testBNN, data[:s], data[:Y])

# ╔═╡ fa3fa6d6-3ed0-43d9-8d19-a8e55f7b6285
weightmats = [layer.weight for layer in aux_BNN]
...
begin
	# Callback for spiking of somas:
	spiking_callback = spiking_cb(network, :soma; θ = 1., T_refractory = 5.)
	# Callbacks for maintaining plateau of synapses:
	plateau_callback = plateau_cb(network, :syn; θ = 1., T = 20.)
end

# ╔═╡ e695d089-b958-4971-bfa3-856760ffba6e
begin
	test_spiking_callback = test_spiking_cb(network, :soma; θ = 1., T_refractory = 5.)
	test_plateau_callback = test_plateau_cb(network, :syn; θ = 1., T = 20.)
end
...
# ╔═╡ 08019dd4-271f-4088-a917-5adae79dfc5c
test_sol = solve(prob, Tsit5();
		reltol = 1e-2, abstol = 1e-2,
        callback = CallbackSet(test_spiking_callback, test_plateau_callback)
);
...
# ╔═╡ e62c77e5-0e56-4f45-b640-9f641a5774b1
layer_plots[3][1]
...
layer_outputs = [spike_finder(network, sol, V_syms; threshold=.999) for V_syms in V_variables]
...
# ╔═╡ c235a033-5f0e-466a-b132-89a098211ed5
aux_accuracy
...
accplot = let
	...
	hlines!(ax1, [aux_accuracy * 100], color = :red, label = "BNN"; xmin = 0.045, 
	xmax = 0.95)
...
# ╔═╡ 8ae023d0-298e-44d8-bd90-178233538b4b
aux_BNN
